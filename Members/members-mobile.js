...
(function () {
  ...
  /* Certification is not on every deployment — it is still STAGE-only while the
     programme is being built, and a Learning tab pointing at a 404 is worse than
     no tab. Rather than keep two divergent copies of this file, ask the page: the
     nav menu is on every member page and lists certification only where it exists.
     One file, correct in both places, and it self-corrects the day certification
     is promoted. */
  function hasCertification() {
    return !!document.querySelector('.nav-menu a[href="certification.html"]');
  }
  ...
  function buildTabBar() {
    ...
    var on = currentTab();
    var cert = hasCertification();
    var html = '<nav class="mm-tabbar" role="navigation" aria-label="Member portal">';
    TABS.filter(function (t) { return t.key !== 'learn' || cert; }).forEach(function (t) {
  ...
  function buildHome() {
    ...
    var html = '<div class="mm-home">'
      ...
      + chips
      /* No certification on this deployment means no training to open, so the
         screen simply has no primary action rather than a button to nowhere. */
      + (hasCertification()
