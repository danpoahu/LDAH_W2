// One-off: send Virginia O'Toole an IN-PERSON 5-day reminder for
// Connect-Gen on 2026-04-27. She has a special accommodation to
// attend at the LDAH office instead of on Zoom. The scheduled job
// already sent a Zoom-based reminder because her signup defaults
// to the virtual session and has no per-date mode override.
//
// This script does ONLY #2 from the plan: send the corrected email.
// It does NOT modify her signup doc (no modeOverride, no remindersSent).
...
function buildInPersonReminderHtml({ recipientName, eventTitle, dayName, dateFormatted, startTime, endTime, locationLabel, surveyUrl }) {
  ...
  const seeYouLine = `<p style="margin:0 0 16px;font-size:16px;color:#333333;line-height:1.5;">
      We are looking forward to seeing you in person on ${dayName}, ${dateFormatted}${timeLine} at ${locationLabel}.
    </p>`;

  const accomNote = `<p style="margin:0 0 16px;font-size:16px;color:#333333;line-height:1.5;">
      As arranged, you will be joining us <strong>at the LDAH office</strong> for this session rather than on Zoom.
    </p>`;
...
async function main() {
  ...
  const subject = `Reminder: ${eventTitle} -- ${dayName}, ${formatted} (In Person at LDAH)`;
  ...
  await db.collection("emailLog").add({
    ...
    note: "Manual send — corrected from Zoom to in-person at office (Virginia special accommodation)",
  });
